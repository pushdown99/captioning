...
cfg.JSONS42    = join(cfg.ROOT, 'download/4-2')
cfg.JSONS43    = join(cfg.ROOT, 'download/4-3')
cfg.JSONS44    = join(cfg.ROOT, 'download/4-4')
...
instances = dict()
for i, p in enumerate(tqdm(glob(join(cfg.JSONS42, '**/*.json'), recursive=True), desc='jsons ')):
